...
// SYNTAXE DATE FORMAT FRANCAIS DANS LE MAP : new Date(oneArticles.date).toLocaleDateString()


// TABLEAU D'OBJETS "articles" 
const articles = [
  {
    "id": 1,
...
const Articles = () => {
    ...
    // METHODE ".filter()" COMBINEE A LA METHODE ".includes()"
    // "filter" PERMET D'APPLIQUER DES CONDITIONS AUX ELEMENTS CONTENUS DANS LE TABLEAU
    // "filter" FILTRE LE TABLEAU "articles" POUR NE GARDER QUE LES OBJETS "article" QUI "includes"(CONTIENNENT) DANS LEURS PROPRIETES "content" UN MOT EQUIVALENT A CELUI STOCKER DANS "search"
    // LA VARIABLE "selected" STOCKE LE NOUVEAU TABLEAU FILTRER
    // CE TABLEAU FILTRER SERA MAPPER DANS LE RETURN POUR AFFICHER SEULEMENT LES ARTICLES LIES A LA RECHERCHE DE L'UTILISATEUR
    const handleChange = (e) => {
      const search = e.target.value
      setUserSearch(search)
      let selected = articles.filter(art => art.content.toLowerCase().includes(search)
      || art.title.toLowerCase().includes(search) 
      || art.summary.toLowerCase().includes(search)
      || art.author.toLowerCase().includes(search)
      || art.category.toLowerCase().includes(search)
      )
      setFilteredArticles(selected) 
    }
      ...
      <div className="min-h-screen bg-[#0E1217]">
        <div className="container mx-auto px-4 pt-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
           {!filteredArticles && (<LatestArticles arrayArticles={articles}/>) }
           {filteredArticles && (
           <>
             {filteredArticles.map ((oneArticles, index) => (
            ...
            <span className="absolute top-0 right-0 bg-[#E11D48] text-white text-xs font-semibold px-3 py-1 rounded-bl-lg">
              ...
              <div className="flex justify-between items-center p-6 pt-0">
                ...
                <span className="text-sm text-gray-600">{new Date(oneArticles.date).toLocaleDateString('fr-FR')}</span>  
              </div>
